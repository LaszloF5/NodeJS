...
fetch(api_url)
  .then((data) => data.json())
  .then((foods) => {
    const container = document.querySelector(".js-dessert-container");
    const dessertRecipes = foods.filter(
      (dessert) => dessert.dishType === "Desszert"
    );
  ...
  .catch((error) => {
    const errorContainer = document.querySelector(".js-dessert-container");
