...
fetch(api_url)
  .then((data) => data.json())
  .then((foods) => {
    const container = document.querySelector(".js-dinner-container");
    const lunchRecipes = foods.filter((lunch) => lunch.dishType === "Vacsora");
  ...
  .catch((error) => {
    const errorContainer = document.querySelector(".js-dinner-container");
