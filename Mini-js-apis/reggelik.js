...
fetch(api_url)
  .then((data) => data.json())
  .then((foods) => {
    const container = document.querySelector(".js-reggelik-container");
    const breakfastRecipes = foods.filter(
      (breakfast) => breakfast.dishType === "Reggeli"
    );
  ...
  .catch((error) => {
    const errorContainer = document.querySelector(".js-reggelik-container");
