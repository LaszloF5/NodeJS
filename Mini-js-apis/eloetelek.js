...
fetch(api_url)
  .then((data) => data.json())
  .then((foods) => {
    const container = document.querySelector(".js-eloetelek-container");
    const appetizerRecipes = foods.filter(
      (appetizer) => appetizer.dishType === "Előétel"
    );
    appetizerRecipes.forEach((recipe) => {
      const recipeHTML = `
  ...
  .catch((error) => {
    const errorContainer = document.querySelector(".js-eloetelek-container");
